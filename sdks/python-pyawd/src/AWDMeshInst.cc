 ...
 * Member dictionary
*/
PyMemberDef pyawd_AWDMeshInst_members[] = {
    { "materials", T_OBJECT_EX, offsetof(pyawd_AWDMeshInst, materials), READONLY,
        "Materials for this mesh instance. Materials at indices that \
            exceed the number of sub-meshes for this mesh will be ignored." },
    { NULL }
};
...
PyTypeObject pyawd_AWDMeshInstType = {
    ...
    pyawd_AWDMeshInst_members,              // tp_members 
    0,                                      // tp_getset 
    0,                                      // tp_base 
    0,                                      // tp_dict 
    0,                                      // tp_descr_get 
...
void
pyawd_AWDMeshInst__prep(pyawd_AWDMeshInst *self)
{
    int i;
    char *name;
    int name_len;
    ...
    mtx = NULL;
    if (self->transform)
        mtx = ((pyawd_AWDMatrix4*)self->transform)->raw_data;
    
    awd_md = NULL;
    if (self->mesh_data)
        awd_md = ((pyawd_AWDMeshData*)self->mesh_data)->ob_data;
    
    name = PyString_AsString(self->name);
    name_len = PyString_Size(self->name);
    self->lawd_obj = new AWDMeshInst(name, name_len, awd_md, mtx);

    num_materials = PyList_Size(self->materials);
    for (i=0; i<num_materials; i++) {
        pyawd_AWDMaterial *mat;

        mat = (pyawd_AWDMaterial *)PyList_GetItem(self->materials, i);
        self->lawd_obj->add_material(mat->lawd_obj);
    }
}
